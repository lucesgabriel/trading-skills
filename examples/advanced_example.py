"""
Advanced usage example for the pattern scanner skill.
"""
...
from html_generator import generate_html_report  # type: ignore  # noqa: E402
...
def main() -> None:
    ...
    print(f"Senal: {confluence['signal']} -> {confluence['primary_probability']:.1f}%")
    print(f"Reporte HTML: {report_path}")
    if confluence['confluence_factors']:
        print("Factores principales:")
        for factor in confluence['confluence_factors'][:5]:
            print(f"  - {factor}")
